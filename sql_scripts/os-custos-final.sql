...
,DA AS (
    SELECT 
         TO_DATE('01/01/2022 00:00:01','DD/MM/YYYY HH24:MI:SS') DI
        ,TO_DATE('31/12/2022 23:59:59','DD/MM/YYYY HH24:MI:SS') DF
        FROM dual
)
--GRUPO CCUSTO Numero de Minutos trabalhados
 ...
 FROM ORDEM_SERVICO A, CENTRO_CUSTO CC, GRUPO_CENCUSTO GC,
      (SELECT * FROM U) UE
 WHERE A.DTA_CRIACAO_OS BETWEEN (SELECT da.di FROM da) AND (SELECT da.df FROM da)
 ...
 FROM APONTAMENTO_ATIVIDADE AA, FUNCIONARIO_ORDEM_SERVICO FOS,
      ...
      (SELECT * FROM U) UE
 WHERE AA.DTA_HOR_INICIO_ATIVIDADE BETWEEN (SELECT da.di FROM da) AND (SELECT da.df FROM da)
 ...
 FROM ORDEM_SERVICO OS,  CENTRO_CUSTO CC, GRUPO_CENCUSTO GCC,
      (SELECT * FROM U) UE
 WHERE OS.DTA_HOR_FINALIZACAO BETWEEN (SELECT da.di FROM da) AND (SELECT da.df FROM da)
   AND OS.COD_SITUACAO_OS IN (4,5,6,13)
 ...
 FROM ORDEM_SERVICO OS,  CENTRO_CUSTO CC, GRUPO_CENCUSTO GCC,
      (SELECT * FROM U) UE 
 WHERE OS.DTA_CRIACAO_OS BETWEEN (SELECT da.di FROM da) AND (SELECT da.df FROM da)  
   AND OS.DTA_HOR_FINALIZACAO BETWEEN (SELECT da.di FROM da) AND (SELECT da.df FROM da)
   AND OS.COD_SITUACAO_OS IN (4,5,6,13)
   AND OS.COD_UNIDADE_EXECUTANTE=UE.COD_UNIDADE_EXECUTANTE
   AND OS.COD_CENCUSTO = CC.COD_CENCUSTO
   AND CC.COD_GRUPO = GCC.COD_GRUPO
 ...
 FROM DOCUMENTO_FINANCEIRO DF, AGENDA_AUT_ENTREGA_SERVICO AGS, AUTORIZACAO_ENTREGA_SERVICO AES, COTACAO_PEDIDO_SERVICO CPS,
      PEDIDO_SERVICO PS, TRIAGEM_ORDEM_SERVICO TOS, ORDEM_SERVICO OS, GRUPO_CENCUSTO GCC,
       CENTRO_CUSTO CC,
      (SELECT * FROM U) UE
 WHERE DF.DTA_EMISSAO BETWEEN (SELECT da.di FROM da) AND (SELECT da.df FROM da)
 ...
 FROM ITEM_REQUISICAO_MATERIAL IRM, REQUISICAO_MATERIAL RM, COMPLEMENTO_REQUISICAO CR,
      TRIAGEM_ORDEM_SERVICO TOS, ORDEM_SERVICO OS, GRUPO_CENCUSTO GCC,
       CENTRO_CUSTO CC,
      (SELECT * FROM U) UE
 WHERE IRM.DTA_MOV_CONTABIL BETWEEN  (SELECT da.di FROM da) AND (SELECT da.df FROM da)
