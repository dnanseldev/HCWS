...
select  ROWID
        ,t.seq_insumo_unitario
        ,t.idf_operacao
        ,t.* from ESTOQUE.SOLICITACAO_INSUMO_ITEM t
where t.seq_solicitacao_insumo = 60629
--AND t.seq_solicitacao_insumo_item = 1029735
AND t.seq_insumo_unitario = 165754
ORDER BY t.seq_insumo_unitario, t.dta_hor_cadastro DESC;

select COUNT(*) QTD, T.SEQ_INSUMO_UNITARIO 
     from ESTOQUE.SOLICITACAO_INSUMO_ITEM t
WHERE 1 = 1
AND t.seq_solicitacao_insumo = 60629
