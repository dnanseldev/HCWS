...
/*
 ...
 - 62795 MZAMBONATO MARCIO ZAMBONATO
*/
SELECT *
  FROM USUARIO U
 WHERE 1 = 1
       AND u.num_user_banco = 62795;--5744;
...
/*V2*/
SELECT   U.NOM_USUARIO || ' ' || U.SBN_USUARIO FUNCIONARIO_APONTAMENTO,
         UE.NOM_UNIDADE_EXECUTANTE CENTRO_TRABALHO,
         OS.NUM_ORDEM_SERVICO,
         OS.ANO_ORDEM_SERVICO,
         TSO.DSC_SITUACAO_OS SITUACAO_OS,
         OS.COD_CENCUSTO CC_SOLICITANTE,
         ...
         ATV.CPL_ATIVIDADE
  FROM  ORDEM_SERVICO OS 
  JOIN LOCAL LC ON LC.COD_LOCAL = OS.COD_LOCAL
  LEFT JOIN BEM_PATRIMONIAL BP USING(NUM_BEM)
  ...
  WHERE 1 = 1
  --AND U.NUM_USER_BANCO = 5744--62795
  --AND fue.cod_unidade_executante = 127
  --AND U.NUM_USER_BANCO = 5744--6
  AND U.NOM_USUARIO_BANCO = 'MZAMBONATO'
  --AND OS.NUM_ORDEM_SERVICO = 42654
  --AND OS.ANO_ORDEM_SERVICO = 2023
  ORDER BY OS.DTA_CRIACAO_OS DESC;  
------------------------------
